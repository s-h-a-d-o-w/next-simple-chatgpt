...
test("Auto-scroll snaps to bottom when user does not scroll", async ({ page }) => {
  await page.goto("/");

  await mockChatResponse(page, {
    text: Array.from({ length: 50 }, () => Math.random().toString()).join("\n"),
  });
  await submitPrompt(page, "Please output 50 lines with 1 random word on each line.");
...
test("Auto-scroll pauses when user scrolls up during streaming, and resumes on next message", async ({
  ...
  await submitPrompt(page, "Please output 10 lines with 1 random word on each line.");
  await expect.poll(() => bottomDistance(page)).toBeLessThanOrEqual(0);
});
